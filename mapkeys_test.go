...
func TestUnmarshalENVOverlaysExistingMapStruct(t *testing.T) {
	assert := assert.New(t)

	t.Setenv("APP_APPS_0_URL", "http://env")
	...
	type cfg struct {
		Apps map[string]app `xml:"apps"`
	}

	config := &cfg{Apps: map[string]app{
		"0": {URL: "http://file", APIKey: "file-key", Name: "from-file"},
	}}
	ok, err := cnfg.UnmarshalENV(config, "APP")
	...
	got := config.Apps["0"]
	assert.Equal("http://env", got.URL)
	assert.Equal("file-key", got.APIKey)
	assert.Equal("from-file", got.Name)
}

func TestUnmarshalENVOverlaysExistingMapPointer(t *testing.T) {
	assert := assert.New(t)
	...
	config := &cfg{Apps: map[string]*app{
		"0": {URL: "http://file", APIKey: "file-key"},
	}}
	ok, err := cnfg.UnmarshalENV(config, "APP")

	require.NoError(t, err)
	assert.True(ok)
	require.NotNil(t, config.Apps["0"])
	assert.Equal("http://env", config.Apps["0"].URL)
	assert.Equal("file-key", config.Apps["0"].APIKey)
}

func TestUnmarshalENVOverlaysNilMapPointer(t *testing.T) {
	assert := assert.New(t)
	...
	config := &cfg{Apps: map[string]*app{"0": nil}}
	ok, err := cnfg.UnmarshalENV(config, "APP")
	...
	require.NotNil(t, config.Apps["0"])
	assert.Equal("http://env", config.Apps["0"].URL)
	assert.Empty(config.Apps["0"].APIKey)
}

func TestUnmarshalENVOverlaysMapSliceIndex(t *testing.T) {
	assert := assert.New(t)

	t.Setenv("APP_FOLDERS_watch_EXCLUDE_PATH_0", "/c")
	...
	type cfg struct {
		Folders map[string]folder `xml:"folders"`
	}

	config := &cfg{Folders: map[string]folder{
		"watch": {ExtractPath: "/file", ExcludePaths: []string{"/a", "/b"}},
	}}
	ok, err := cnfg.UnmarshalENV(config, "APP")
	...
	got := config.Folders["watch"]
	assert.Equal("/file", got.ExtractPath)
	assert.Equal([]string{"/c", "/b"}, got.ExcludePaths)
}
